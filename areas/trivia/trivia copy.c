...
void startup()
{
  ...
  round = 50;
  players = ([ ]);
  players += ([ "bebop": 10; 0; 0,]);
  players += ([ "rocksteady": 10; 0; 0,]);
  ...
  if(sizeof(new_questions) < 1){
    tell_object(GAMEMASTER, "Question list empty! API Error!\n");
    return;
  }
...
int do_answer(string who, string what)
{
  ...
  {
    // waiting += ({ who, });
    tell_object(GAMEMASTER, 
      "\nTriviaBot tells you: The answer is $CYAN"
      +capitalize(full_answer)+" ("+capitalize(letter_answer)+")$OFF\n");

    tell_object(GAMEMASTER, 
      "TriviaBot tells you: $GREEN"+capitalize(who)
      +"$OFF guessed $YELLOW"+capitalize(what)+"$OFF\n\n");
    // number_guessed++;
    got_wrong(who);
    return 1;
  }
...
int add_player(string who)
{
  ...
  players += ([ who: 0; 0; 0,]);
  tell_room(environment(this_object()), parse_ansi("TriviaBot announces: $CYAN"+capitalize(ob->QN)+"$OFF has joined Trivia!\n"));
  tell_object(ob, "TriviaBot tells you: type 'tell trivia /help' for more information.$OFF\n");
  return 1;
}
...
void display_question()
{
  ...
  say(parse_ansi(
    "\n$UNDERL$BOLDQuestion for round:$OFF$BOLD$YELLOW "+round+"/"+MAX_ROUNDS+"$OFF$BOLD\n"));
...
int check_points()
{
  ...
  {
    if (round==MAX_ROUNDS && leader_score<=players[names[i], 0]){
      ...
      tell_object(who, 
        parse_ansi("$BOLDSorry, you $REDlost$WHITE. Better luck next time!\n"));
    }
...
//Indexing on illegal val?
  ...
  if(final_winner == "tie")
  {
    ...
    say(parse_ansi(
      "\n$BOLD$REDSUDDEN DEATH$OFF WILL BEGIN IN $GREEN1$OFF MINUTE!$OFF\n"
      ...
      //HEARER->listen("The Trivia game has ended. "+tie_winners+" are the champions!\n");
  }
