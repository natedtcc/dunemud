...
void reset(int arg)
{
  ...
  if (!arg){
    ...
    add_exit("west", ROOM_DIR + PACK + "pack3.c");
    add_exit("east", ROOM_DIR + PROC + "proc11.c");

    add_item(
      "doors",
        "There's two doors, one to the west and one two the east. You also see what\n"
        +"seems to be big metallic door way down the hallway.");
  }
...
check()
{
  // Wizards can move through here freely
