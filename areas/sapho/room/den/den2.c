...
void reset(int arg)
{
  ...
  if (!arg){
    ...
    add_exit("south", ROOM_DIR + "dhall2.c");
    add_exit("west", ROOM_DIR + DEN +"den1.c");
    add_exit("east", ROOM_DIR + DEN + "den3.c");
    add_exit("north", ROOM_DIR + DEN + "den5.c");
    ...
    if (!present("board")) {
      ...
      board->set_name(parse_ansi("$BOLD$REDSapho Den$OFF"));
      board->set_save_file("/d/Imperial/bebop/sapho/board");
      board->set_article("The");
      board->set_remove_level(250);
      board->secure();
