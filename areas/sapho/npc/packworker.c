...
void reset(status arg)
{
  ...
  if (!arg) {
    ...
    improve_skill("Hand to Hand", 500);
    ...
    set_combat_chats (20, ({
      ({ "The worker tries to intimidate you by screaming!", "echo" }),
      ({ "The worker goes for the leg sweep.", "echo" })
      }));

    armor = clone_object( OBJ_DIR + EQUIP + "mask.c");
